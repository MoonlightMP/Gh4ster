...
case $menusel in
		...
		pause
		clear ;;	
	"Update Kali Sana 2.0 to Kali 2017.3")
		clear
		echo -e "\033[32mUpdating Kali Sana to Kali Linux 2017.3\033[m"
		rm /etc/apt/sources.list
		echo "" >> /etc/apt/sources.list
		echo 'deb http://http.kali.org/kali kali-rolling main non-free contrib' >> /etc/apt/sources.list
		echo 'deb-src http://http.kali.org/kali kali-rolling main contrib non-free' >> /etc/apt/sources.list
...
######## Install htop, iotop, collectl, lsof, monit, nmon, sysstat
function installMonitoringtools {
	echo -e "\e[1;31mThis option will install htop, iotop, collectl, lsof, monit, nmon, sysstat\e[0m"
	echo -e ""
	echo -e "Do you want to install it ? (Y/N)"
			read install
			if [[ $install = Y || $install = y ]] ; then	
				echo -e "\033[31m====== Installing monitoring tools ======\033[m"
				sleep 2
				apt-get install htop iotop collectl lsof monit nmon sysstat -y
				xterm -e apt-get install nethogs iftop vnstat acct suricata iptraf -y
...
######## TCP SET
function installTCPSET {
	echo -e "\e[1;31mThis option will hide some tags of tcp\e[0m"
	echo -e ""
	echo -e "Do you want to install it ? (Y/N)"
			read install
			if [[ $install = Y || $install = y ]] ; then	
				echo -e "\033[31m====== Let's hide  ======\033[m"
				sleep 2
				xterm -e echo "net.ipv4.tcp_timestamps = 0" > /etc/sysctl.d/tcp_timestamps.conf
				xterm -e sysctl -p /etc/sysctl.d/tcp_timestamps.conf			
				echo -e "\033[32m====== Done Installing ======\033[m"
			else
				echo -e "\e[32m[-] Ok,maybe later !\e[0m"
			fi
...
######## Banner of Wellcome logo
function installWellcomeLogo {
	echo -e "\e[1;31mThis option will install Wellcome logo\e[0m"
	echo -e ""
	echo -e "Do you want to install it ? (Y/N)"
			read install
			if [[ $install = Y || $install = y ]] ; then	
				echo -e "\033[31m====== Installing Banner of Wellcome logo   ======\033[m"
				sleep 2
				apt-get install figlet
                apt-get install sysvbanner			
				echo -e "\033[32m open terminal -> type leafpad .bashrc -> in the end -> figlet -f slant name\033[m"
				echo -e "\033[32m====== Done Installing ======\033[m"
			else
				echo -e "\e[32m[-] Ok,maybe later !\e[0m"
			fi
...
######## optimization
function installoptimization {
	echo -e "\e[1;31mThis option will install optimization for your system\e[0m"
	echo -e ""
	echo -e "Do you want to install it ? (Y/N)"
			read install
			if [[ $install = Y || $install = y ]] ; then	
				echo -e "\033[31m====== Installing optimization   ======\033[m"
				sleep 2
				apt-get autoremove 
                apt-get autoclean
				...
				xterm -e prelink --all
				echo -e "\033[32m Open terminal -> leafpad /etc/default/prelink - > PRELINKING=yes -> save & close \033[m"
				echo -e "\033[32m====== Done Installing ======\033[m"
			else
				echo -e "\e[32m[-] Ok,maybe later !\e[0m"
			fi
...
function installecryptfs-utils {
	echo -e "\e[1;31mThis option will install ecryptfs-utils!\e[0m"
	echo -e ""
	echo -e "Do you want to install it ? (Y/N)"
			read install
			if [[ $install = Y || $install = y ]] ; then	
				echo -e "\033[31m====== Installing ecryptfs-utils ======\033[m"
				sleep 2
...
function installsecure-delete {
	echo -e "\e[1;31mThis option will install secure-delete!\e[0m"
	echo -e ""
	echo -e "Do you want to install it ? (Y/N)"
			read install
			if [[ $install = Y || $install = y ]] ; then	
				echo -e "\033[31m====== Installing secure-delete ======\033[m"
				sleep 2
...
######## Install ipkungfu
function installipkungfu {
	echo -e "\e[1;31mThis option will install ipkungfu!\e[0m"
	echo -e ""
	echo -e "Do you want to install it ? (Y/N)"
			read install
			if [[ $install = Y || $install = y ]] ; then	
				echo -e "\033[31m====== Installing ipkungfu ======\033[m"
				sleep 2
			    apt-get install ipkungfu		
				echo -e "\033[32mOpen terminal - > nano /etc/ipkungfu/ipkungfu.conf \033[m"
				echo -e "\033[32mLOCAL_NET="127.0.0.1"\033[m"
                echo -e "\033[32mGATEWAY=0\033[m"
                echo -e "\033[32mFORBIDDEN_PORTS="135 137 139"\033[m"
                echo -e "\033[32mBLOCK_PINGS=1\033[m"
                echo -e "\033[32mSUSPECT="DROP"\033[m"
                echo -e "\033[32mKNOWN_BAD="DROP"\033[m"
                echo -e "\033[32mPORT_SCAN="DROP"\033[m"
				echo -e "\033[32mThen - > leafpad  /etc/default/ipkungfu - > IPKFSTART = 1 \033[m"
                echo -e "\033[32mType -> sudo ipkungfu \033[m"
                echo -e ""
                echo -e "\033[32mnano /etc/systcl.conf\033[m"
                echo -e "\033[32mnet.ipv4.conf.all.accept_redirects=0\033[m"
                echo -e "\033[32mnet.ipv6.conf.all.accept_redirects=0\033[m"
                echo -e "\033[32mnet.ipv4.tcp_syncookies=1\033[m"
                echo -e "\033[32mnet.ipv4.tcp_timestamps=0\033[m"
                echo -e "\033[32mnet.ipv4.conf.all.rp_filter=1\033[m"
                echo -e "\033[32mnet.ipv4.tcp_max_syn_backlog=1280\033[m"
                echo -e "\033[32mkernel.core_uses_pid=1\033[m"
                echo -e ""
                echo -e "\033[32m sudo sysctl -p \033[m"
				echo -e "\033[32m====== Done Installing ======\033[m"
			else
				echo -e "\e[32m[-] Ok,maybe later !\e[0m"
			fi
...
\033[m              
\033[31m\033[m"
select menusel in "Errors_Menu" "VPN" "VPN-BOOK" "Monitoring Tools" "Banner of Wellcome logo" "Secure-Delete" "IpKungFu" "Ecryptfs-Utils" "TCP hide" "Install MinGW" "VirtualBox" "Bleachbit" "Sopcast" "GoldenDict" "Java" "Pinta" "ibus" "libreoffice" "knotes" "Tor Browser" "Fix Sound Mute" "Chromium" "Archive-Manager" "Gdebi" "bittorrent client" "Fix VMWare" "YouTube Downloader" "Install Skype" "Fix VLC" "Firefox" "Google Chrome" "Install All" "Back to Main"; do
case $menusel in
	"VPN")
	    installvpn
		pause
	...
	"Secure-Delete")    
	    installsecure-delete
	    pause
	    softwaresandystemtools ;;

	"Ecryptfs-Utils")
	    installecryptfs-utils
	    pause
	    softwaresandystemtools ;;
...
select menusel in   "Masscan" "Dirs3arch" "Install dvwa" "Eyswitness" "install bWAPP" "Maligno" "Imagejs" "Backdoor" "Harness" "RIPS" "Hcon" "Miasm" "Spf" "Impacket" "Nosql-Exploitation-Framework" "MPC" "PyPhisher" "Lumberjackjs" "PACK" "Crackq" "Linception" "ZCR-Shellcoder" "YSO-Mobile-Security-Framework" "WS-Attacker" "DarkCometExploit" "DSJS" "Sleepy-puppy" "Jsgifkeylogger" "Brakeman" "Discover" "Owtf" "Rawr" "Empire" "Linux Exploit Suggester" "Shell Code Tools" "Lazagne" "Daws" "Windows Exploit Suggester" "Pompem" "Morpheus" "commix" "bettercap" "Commix" "seclist" "Fluxion" "Mitmf" "Backdoor-Factory" "Wig" "Spade" "Nettool" "Thad0ctor" "Hackpack" "Atscan" "OpenDoor" "Whitewidow" "Unicorn" "Metagofil" "XSSER" "Pyobfuscate" "NetRipper" "Harness" "Wifiphiser" "Weeman" "Yamas" "Fatrat" "Install All" "Back to Main"; do
case $menusel in 
...
select menusel in "Install All" "Back to Main"; do
case $menusel in 
...
function mitm
{
	echo -e "\e[1;31mThis option will Start MITM!\e[0m"
	echo -e "Do you want to Start ? (Y/N)"
			read install
			if [[ $install = Y || $install = y ]] ; then	
				echo -e "\033[31m====== MITM ======\033[m"
				sleep 2
				chmod +x mitm.sh
				./mitm.sh
			else
...
\______   \_______ __ ___/  |_  ____   \_   _____/__________   ____  ____  
 |    |  _/\_  __ \  |  \   __\/ __ \   |    __)/  _ \_  __ \_/ ___\/ __ \ 
 |    |   \ |  | \/  |  /|  | \  ___/   |     \(  <_> )  | \/\  \__\  ___/ 
...
select menusel in  "MITM" "Brute" "Automation" "USE FORCE OF Metasploit" "Back to Main"; do
case $menusel in 
...
\033[m          Script by Moonlight
            Version : 6.6.6 \033[32m$version\033[m
\033[32m\033[m"

select menusel in "Update Kali" "Desktop Manager" "Update Tools" "Software and System Tools" "Install Hacking Tools" "Kali-Tools" "LET SET HACK" "Check Gh4st-Update" "EXIT PROGRAM"; do
case $menusel in
    ...
    "Check Gh4st-Update")
	  ...
	  firefox https://github.com/Scorpioni4e/Gh4ster
	  pause
	  clear ;;
